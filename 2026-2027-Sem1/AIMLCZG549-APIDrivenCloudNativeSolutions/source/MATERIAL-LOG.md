...
- **Source-figure fidelity correction (03 Sep 2026).** A direct pptx media audit (`a:blip`/PICTURE-shape inspection, not a visual skim) found that S04 and S05 both contained genuine embedded content images that had been recreated as authored SVGs instead of preserved as PNG, violating the fidelity rule. Fixed: S04's 7 recreated figures (data deluge context, Big Data 5 V's, Data Science Venn, AI/ML/DS convergence, ML production loop, hierarchy of needs, Big Data life cycle) and S05's 3 (canonical architecture, functional architecture, GitHub Actions vocabulary) now embed the actual source-slide PNGs; the superseded SVGs were deleted. S04 was also missing figures entirely for 3 named source figures (ETL foundation, one-time forecast, and the "Hidden Technical Debt in ML Systems" NeurIPS figure, slides 19/20/21) — these were added as new preserved PNGs. Slides confirmed decorative/non-substantive and correctly left unpreserved: S04 slide 18 (video-thumbnail play button) and slide 8's three generic stock photos (data storage, AI brain, computation icon). `AGENTS.md`'s completion checklist now has its own line requiring this media-level audit, separate from "every concept has a diagram", so authoring an explanatory SVG can no longer be mistaken for satisfying source-figure fidelity.
