...
**Hard completion gate: all handout topics must be covered.** A session note is **not complete** until every topic and sub-topic in that session's handout Learning Plan row is explicitly taught in the note. Missing even one bullet means the note remains incomplete, regardless of how polished the rest of the note is, how good the deck was, or whether the missing item appears in a later/earlier session. Before marking a note done, make a bullet-by-bullet handout checklist and confirm each item maps to a clear heading, subsection, table row, worked example, or diagram inside that same session note.

**Create session notes from the handout, not from uploaded material.** The handout Learning Plan row decides whether a note exists, its session number, title, boundaries, and required sub-topics. Uploaded PPT/PDF/papers/transcripts are **teaching inputs** used to explain that handout row better; they do **not** create new note scope, rename sessions, merge sessions, split sessions differently, or add exam-scoped topics by themselves. If an upload contains extra material, older-offering material, or multiple sessions, use only the parts that match the handout row for the current note and log the mismatch in `source/MATERIAL-LOG.md`.

**Always read the handout-cited sources for that row before calling a note complete.** For every create/review/update of a session note, first read the exact handout row, then read/extract the cited textbook chapter(s), paper(s), public spec(s), or web references named in that row. Do this even when the deck looks sufficient. The deck sets instructor emphasis; the handout references supply mechanism, examples, and edge cases. If a cited source is missing, blocked, or out of reach, mark the gap in `source/MATERIAL-LOG.md` and the watchlist; do not silently skip it or say the note is fully reviewed.
