...
**Build-up and visual completeness gate.** Repeated-title or animation/build slides are not empty rows. If a slide adds a token, prompt fragment, generated word, code state, table, diagram panel, arrow, or other visual step, record that addition in the temporary fact/visual ledger and give it a deliberate home in the note. Do not collapse slides 4–12 of an autoregressive example, a tokenization sequence, or a serialized prompt into one generic paragraph merely because the extracted title repeats. For every substantive native media object, record `source slide → note section → native asset or Markdown transcription`; a passing text checker does not satisfy this gate. Before finalizing, compare the note against the complete slide sequence and the native-media contact sheet, then confirm that every source example and instructional figure is either preserved directly, transcribed, routed intentionally, or marked as a justified omission.
