...
- **Attribute with a *durable* reference, not a slide number.** Under each topic heading, a short `*Reference: …*` line pointing at what the reader would actually **go read to learn more** — the spec, official docs, the textbook chapter, the canonical article, a link. A bare "slides 13–14" is useless once the deck is gone; a reference outlives the course, which is what a knowledge base needs. **Where a concept genuinely has only the deck behind it** (common in 549 from S4 on), say so plainly — *"deck only; no durable source behind this"* — rather than inventing a reference. Attribution is a light line, never structure.
