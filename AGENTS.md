...
**Every concept in every note — session notes and `_shared/` notes alike — has at least one clear diagram: Mermaid or authored SVG.** Use Mermaid only for clean structures that render well. Use authored SVG when the source figure's teaching shape, grouped layout, labels, miniature bars, matrix/table-like visuals, probability bars, or print readability matters. Do not keep a Mermaid diagram beside an SVG if both teach the same picture; the clearer one replaces the weaker one.
